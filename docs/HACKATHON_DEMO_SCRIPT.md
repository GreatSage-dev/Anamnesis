...
> 
> *When a new transaction arrives, Anamnesis queries CockroachDB in under 25 milliseconds via dual-path retrieval — matching exact wallet history AND semantic vector similarity. Then, **Amazon Bedrock Claude 3.5 Sonnet** synthesizes a verdict, explicitly citing historical precedents."*
...
> 
> *Now watch **Pass 2**. Five minutes later, the same wallet attempts another transaction. Anamnesis instantly queries CockroachDB, retrieves Pass 1's exact decision UUID, and feeds it to Claude 3.5 Sonnet.*
> 
> *Look at the reasoning text in Pass 2: It explicitly cites Precedent `#990d382f`, notes that this is a repeat check on the same counterparty, and reinforces caution! Memory caught what point-in-time scanning missed."*
...
- Point out the **Recent Memory Stream** thin-bar activity log and the **Fail-Open Resilient** badge.
...
> 
> *Second, **Fail-Open Downtime Resilience**: If CockroachDB Cloud or Bedrock ever experiences network latency, Anamnesis falls back to deterministic on-chain signals in under 50 milliseconds tagged with fallback reasoning. Agent payments on X Layer never freeze."*
...
<div align="center">
  <sub>Script formatted for CockroachDB × AWS Hackathon video submission</sub>
</div>
